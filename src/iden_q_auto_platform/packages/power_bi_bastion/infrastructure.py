...
from iden_q_auto_platform.models.tenants.cross_platform import CrossPlatform
...
class PowerBiBastion(Construct):
    ...
    Motivation: The data prod environment should not be exposed to the internet.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tenant.environment == AppEnvironment.PROD:
            environment_name = f"{tenant.company}-{tenant.product}-{tenant.environment.value}-{CrossPlatform.POWERBI.value}"

...
class ServiceLoadBalancerLoggingBuilder(Construct):
    def _build_load_balancer_logging(
        ...
        ALB_LOGS_BUCKET_NAME = (
            f"{tenant.company}-{tenant.product}-{tenant.environment.value}-alb-logs-bucket"
        )
        ...
        ALB_LOGS_DATABASE_NAME = (
            f"{tenant.company}-{tenant.product}-{tenant.environment.value}-alb-logs-db"
        )
        ...
        ALB_LOGS_WORKGROUP_NAME = (
            f"{tenant.company}-{tenant.product}-{tenant.environment.value}-alb-logs-workgroup"
        )
